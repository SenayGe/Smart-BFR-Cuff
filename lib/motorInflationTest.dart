...
class _LopCalibrationScreenState extends State<LopCalibrationScreen> {
     ...
     and press calibrate''',
            ...
            Column(
              children: <Widget>[
                ...
                Text("Your approx LOP is 105 mmHg",style: TextStyle(fontSize: 22, color: Colors.green,fontWeight: FontWeight.w100)),
  ...
  Future<void> listenToEsp() {
    espConnection.input.listen((Uint8List data) async {
      ...
      else if(receivedData.trimLeft().substring(0, 1)=="!"){
        String lop_calculated = receivedData.substring(1).trim();
        Utilities.showSnackBar(scaffoldKey, '$lop');
        bitalinoController.stop();
        bitalinoController.disconnect();
        bitalinoController.dispose();
        espConnection.dispose();
